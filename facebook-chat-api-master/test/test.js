...
var conf = JSON.parse(fs.readFileSync('../facebook-chat-api-master/test/test-config.json', 'utf8'));
var credentials = {
...
describe('Login:', function() {
  ...
  it('should create a chat', function (done){
    var body = "new-chat-" + Date.now() + " 의도는 짱짱맨";
    var inc = 0;
  ...
  it('should send text message object (group)', function (done){
    var body = "text-msg-obj-" + Date.now() + "이거시 테스트다";
    listen(done, msg =>
      msg.type === 'message' &&
      msg.body === body &&
      msg.isGroup === true
...
//        return (
  ...
  it('should get the right user info', function (done) {
    api.getUserInfo(userID, function(err, data) {
      ...
      console.log("right user 정보를 달라 "+data[userID]);
      assert(user.name);
    ...
    api.getUserInfo(userIDs[0], function(err, data) {
      ...
      api.getUserID(user.name, function(err, data) {
        ...
        assert(data.some(function(val) {
          console.log("user ID 정보를 달라 "+userIDs[0]);
